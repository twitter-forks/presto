        ...
        implements Rule<ProjectNode>
{
    ...
    public Result apply(ProjectNode parent, Captures captures, Context context)
    {
        ...
        System.err.println("----ProjectOffPushDownFieldRule::apply----");
        System.err.println(parent.getAssignments().getExpressions());

        return pruneNestedFields(targetNode.getOutputSymbols(), parent.getAssignments().getExpressions())
                .map(prunedOutputs -> {
                    System.err.println("----process prunedOutputs----");
                    System.err.println(targetNode.getOutputSymbols());
                    System.err.println(prunedOutputs);
                    return prunedOutputs;
                })
                .flatMap(prunedOutputs -> this.pushDownProjectOff(context.getIdAllocator(), targetNode, prunedOutputs))
                .map(newChild -> {
                    System.err.println("----process newChild----");
                    System.err.println(newChild.getClass().getName());
                    System.err.println(newChild.getId());
                    System.err.println(newChild.getOutputSymbols());
                    return newChild;
                })
                .map(newChild -> parent.replaceChildren(ImmutableList.of(newChild)))
                .map(newParent -> {
                    System.err.println("----process newParent----");
                    System.err.println(newParent.getClass().getName());
                    System.err.println(newParent.getId());
                    System.err.println(newParent.getOutputSymbols());
                    System.err.println(((ProjectNode) newParent).getAssignments());
                    return newParent;
                })

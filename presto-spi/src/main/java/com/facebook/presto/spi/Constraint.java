...
public class Constraint<T>
{
    ...
    private final Optional<List<Map.Entry<T, FieldSet>>> fieldSets;
    ...
    public Constraint(Optional<List<Map.Entry<T, FieldSet>>> fieldSets)
    {
        this(TupleDomain.<T>all(), bindings -> true, fieldSets);
    }
    ...
    public Constraint(TupleDomain<T> summary, Predicate<Map<T, NullableValue>> predicate, Optional<List<Map.Entry<T, FieldSet>>> fieldSets)
    {
    ...
    public Optional<FieldSet> getFieldSet(T key)
    {
        return fieldSets
                .flatMap(fieldSets -> fieldSets.stream()
                        .filter(entry -> entry.getKey().equals(key))
                        .findFirst())
                .map(Map.Entry::getValue);
    }

    public Optional<List<Map.Entry<T, FieldSet>>> getFieldSets()
    {
        return fieldSets;
    }

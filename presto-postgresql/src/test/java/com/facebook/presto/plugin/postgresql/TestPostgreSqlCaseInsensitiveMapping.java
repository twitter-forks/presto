        ...
        for (int i = 0; i < nameVariants.length; i++) {
            for (int j = i + 1; j < nameVariants.length; j++) {
                ...
                try (AutoCloseable ignore1 = withSchema(schemaName);
                    ...
                    assertThat(computeActual("SHOW SCHEMAS").getOnlyColumn().filter("casesensitivename"::equals)).hasSize(1); // TODO change com.facebook.presto.plugin.jdbc.JdbcClient.getSchemaNames to return a List
